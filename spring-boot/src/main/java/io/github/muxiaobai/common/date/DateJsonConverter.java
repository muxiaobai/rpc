package io.github.muxiaobai.common.date;
 ...
 * @date 2020/3/25/025 17:23
 * @Json @RequestBody转换配置类

 */
public class DateJsonConverter  extends JsonDeserializer<Date> {
    ...
    public Date deserialize(JsonParser p, DeserializationContext ctxt) throws IOException, JsonProcessingException {
        ...
        if (StringUtils.isNotEmpty(originDate)) {
            try {
                long longDate = Long.parseLong(originDate.trim(),10);
                targetDate = new Date(longDate);
            } catch (NumberFormatException e) {
                try {
                    targetDate = DateUtils.parseDate(originDate, DateJsonConverter.pattern);
